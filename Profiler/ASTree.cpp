...
void AST::mainHeader(std::vector<std::string>& profileNames) {
    ...
    // declaration.
    //Also, add in the profile declaration and the
    //include for profile.hpp

  std::list<AST*>::iterator subtree = child.begin();
  AST * includeChild = new AST(token, "#include \"profile.hpp\"\n");
  child.insert(subtree, includeChild);

  for (std::vector<std::string>::iterator name = profileNames.begin();
       name != profileNames.end(); ++name) {
    std::string profile_name = *name;
    size_t underscore_location = profile_name.find_last_of('_');
    std::string filename = profile_name.replace(underscore_location, 1, ".");
    std::string addition = "profile " + *name + "(\"" + filename + "\");\n";
    if (name == --profileNames.end()) {
      addition = addition + "\n";
    }
    AST * declareChild = new AST(token, addition);
    child.insert(subtree, declareChild);
  }
}
...
void AST::fileHeader(const std::string& profileName) {
    ...
    //For each file profile name, add a new node with a profile 
    // extern declaration.
    //Also, add in the extern profile declaration and the
    //include for profile.hpp
  ...
  size_t underscore_location = profile_name.find_last_of('_');
  std::string filename = profile_name.replace(underscore_location, 1, ".");
  std::string addition = "extern profile " + profileName + ";\n";
  AST * declarationChild = new AST(token, addition);
  child.insert(subtree, declarationChild);
}
...
void AST::mainReport(std::vector<std::string>& profileNames) {
  ...
  for(std::list<AST*>::iterator mainIT = child.begin(); mainIT != child.end(); ++mainIT) {
    if((*mainIT)->tag == "funtion" && (*mainIT)->getChild("name")->getName() == "main") {
      AST* blockTree = (*mainIT)->getChild("block");
      AST* returnTree = blockTree->getChild("return");
      std::list<AST*>::iterator returnIT = returnTree->child.begin();

      for(std::vector<std::string>::iterator namesIT = profileNames.begin(); namesIT != profileNames.end();) {
	    std::string addition = "std::cout << " + *namesIT + " << std::endl;\n ";
	  AST* newNode = new AST(token, addition);

	  returnTree->child.insert(returnIT, newNode);
      }
    }
  }
}
...
void AST::funcCount(const std::string& profileName) {
  ...
  for(std::list<AST*>::iterator mainIT = child.begin(); mainIT != child.end(); ++mainIT) {
    if((*mainIT)->tag == "function") {
      AST* blockTree = (*mainIT)->getChild("block");
      std::list<AST*>::iterator blockIT = blockTree->child.begin();
      std::string funtionName = (*mainIT)->getChild("name")->getName();
      std::string addition = " " + profileName + ".count(__LINE__, \"" + funtionName + "\");";
      AST* newNode = new AST(token, addition);
      blockTree->child.insert(++blockIT, newNode);
    }
  }
}
...
void AST::lineCount(const std::string& profileName) {
  ...
  for(std::list<AST*>::iterator mainIT = child.begin(); mainIT != child.end(); ++mainIT) {
    if(isStopTag((*mainIT)->tag)) {
      continue;
    }
    else if((*mainIT)->tag == "expr_stmt") {
      std::string addition = " " + profileName + ".count(__LINE__);";
      AST* newNode = new AST(token, addition);
      std::list<AST*>::iterator tempIT = mainIT;
      child.insert(++tempIT, newNode);
    }
    else {
      (*mainIT)->lineCount(profileName);
    }
  }
}
